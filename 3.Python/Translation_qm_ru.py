  ...
  for i in tq:
    ...
    if i % checkpoint_steps == 0:
      ...
      if (n_non_improve_val_perplex != None) & (val_amount_test != None):
        ...
        print('Step:', i, '; loss:', train_loss, '; validation loss:', val_loss,'; perplexity:', train_perp, '; validation perplexity:', val_perp)
...
batch_size = 2 # 8
checkpoint_steps = 1
n_non_improve_val_perplex = 2 # Number of subsequent training checkpoints if there is no improvement in validation perplexity
val_amount_test = 1 # Number of tests for validation


# 7.1.UPDATE EMBEDDINGS -----------------------------
...
trainModel(batch_size                = batch_size, 
           ...
           model_path                = MODEL_PATH_QM_EMBEDDING, 
...
trainModel(batch_size                = batch_size, 
           ...
           # steps                     = 10,
           steps                     = int(len(train_all) / batch_size) * 2,
...
trainModel(batch_size                = batch_size, 
           ...
           val_amount_test           = val_amount_test, # Number of tests for validation
           steps                     = 10,
           # steps                     = int(len(train_one) / batch_size) * 3,
...
trainModel(batch_size                = batch_size, 
           ...
           val_amount_test           = val_amount_test, # Number of tests for validation
           steps                     = 10,
           # steps                     = int(len(train_sev) / batch_size),
...
df_all_all["blue_rus"] = df_all_all["krc2rus"].apply(lambda x: bleu_calc.corpus_score(x, [df_all_all['rus'].tolist()]).score)   
df_all_all["blue_krc"] = df_all_all["rus2krc"].apply(lambda x: bleu_calc.corpus_score(x, [df_all_all['krc'].tolist()]).score)   
df_all_all["chrf_rus"] = df_all_all["krc2rus"].apply(lambda x: chrf_calc.corpus_score(x, [df_all_all['rus'].tolist()]).score)   
df_all_all["chrf_krc"] = df_all_all["rus2krc"].apply(lambda x: chrf_calc.corpus_score(x, [df_all_all['krc'].tolist()]).score)   

# Result
df_all_all.copy().groupby("model")[['blue_rus', 'blue_krc', "chrf_rus", "chrf_krc"]].mean()
