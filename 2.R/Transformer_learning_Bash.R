...
FILE_TEXT <- file.path(FILEPATH_SOURCE_PREPARED, "bashqir_orus.parquet")
...
TRANSFORMER_FILE_ALL <- file.path(this.dir(), 'model', TRANSFORMER_NAME_ALL)
...
text_pairs <- FILE_TEXT %>%
  ...
  .[ru !='ГЛАВА ТРЕТЬЯ'] %>% 
  # .[, corpus := NULL] %>% 
  .[, c('ba_leng', 'ru_leng') := lapply(.SD, function(x) {strsplit(x, ' ') %>% sapply(length)}), .SDcols = c('ba', 'ru')] %>% 
  .[(ba_leng <= 100) & (ru_leng <= 100)] %>% 
  .[, .(ba, ru)] %>% 
  .[sample(nrow(.), nrow(.) / 5)]
...
layer_transformer_decoder <- new_layer_class(
  ...
  get_causal_attention_mask = function(inputs) {
    ...
    tf$tile(mask[tf$newaxis, , ],
            tf$stack(c(batch_size, 1L, 1L))) # Добавляем размер пакета в маску, а затем размещаем (rep()) вдоль оси пакета batch_size раз. Возвращенный тензор имеет форму (batch_size, sequence_length, sequence_length)
  },
...
#   get_config = function() {
#     config <- super$get_config()
#     for(name in c("embed_dim", "num_heads", "dense_dim", 'num_layers', 'dropout'))
#       config[[name]] <- self[[name]]
#     config
#   }
# )
...
transformer %>%
  compile(optimizer = opt,# keras$optimizers$Adam(learning_rate = learning_rate, ),
...
transformer1 <- load_model_hdf5(TRANSFORMER_FILE,
                                custom_objects = 
                                  list(PositionalEmbedding = layer_positional_embedding,
                                       TransformerEncoderLayer = layer_transformer_encoder,
                                       TransformerDecoderLayer = layer_transformer_decoder,
